...
int kb900x_get_hw_rtssm_log(const kb900x_config config, kb900x_rtssm_all_logs_t *log) {
  ...
  // Read request
  ret = kb900x_read_smbus_command(config, KB900X_OFFSET_RTSSM_REQ,
                                  &operation_status);
  CHECK_SUCCESS_MSG(
  ...
  while (operation_status != KB900X_FEATURE_REQ_STATUS_SUCCESS &&
         timeout < 100) {
    ret = kb900x_read_smbus_command(config, KB900X_OFFSET_RTSSM_STATUS,
                                    &operation_status);
    CHECK_SUCCESS_MSG(
        ret,
  ...
  ret = kb900x_read_smbus_command(config, KB900X_OFFSET_RTSSM_START_ADDR,
                                  &dccm_buff_addr);
  CHECK_SUCCESS_MSG(ret,
  ...
  // Read DCCM length
  ret = kb900x_read_smbus_command(config, KB900X_OFFSET_RTSSM_LENGTH,
                                  &dccm_buff_length);
  CHECK_SUCCESS_MSG(
      ret,
