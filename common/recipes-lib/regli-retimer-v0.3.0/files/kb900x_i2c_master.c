...
{
    if (data_size > TX_FIFO_DEPTH || data_size <= 0 || data == NULL) {
        KANDOU_ERR(
            "Invalid parameters: make sure data is not null and TX_FIFO_DEPTH < data_size <= 0");
        return -EINVAL;
    }
    int ret = KB900X_E_ERR;
    // Change slave address if needed
    if (slave_addr != i2c_master_slave_addr) {
        ret = i2c_master_set_slave_address(config, slave_addr);
        CHECK_SUCCESS_MSG(ret, "Something went wrong while setting the slave address");
        i2c_master_slave_addr = slave_addr;
    }
    ...
    ret = i2c_master_wait_for_inactivity(config);
    CHECK_SUCCESS_MSG(ret, "Something went wrong while waiting for inactivity");

    // Check status if requested
    if (check) {
        ret = i2c_master_check_status(config, slave_addr, data);
...
{
    if (length > TX_FIFO_DEPTH || length > RX_FIFO_DEPTH || addr == NULL || result == NULL) {
        KANDOU_ERR("Invalid parameters: make sure addr and result are not null and TX_FIFO_DEPTH "
    ...
    if (!skip_addr) {
        // Send address
        ret = i2c_master_write(config, slave_addr, addr, addr_size, check);
    ...
    // Check status if requested
    if (check) {
        ret = i2c_master_check_status(config, slave_addr, addr);
        CHECK_SUCCESS_MSG(ret, "Something went wrong while checking the status");
    }
    ...
    uint32_t tmp_val;
    for (size_t i = 0; i < length; i++) {
        ret = kb900x_read_register(config, ee_IC_DATA_CMD, &tmp_val);
...
int i2c_master_enable(const kb900x_config_t *config, const bool enable, const bool block_fifo)
{
    ...
    int ret = kb900x_write_register(config, ee_IC_ENABLE, value);
    CHECK_SUCCESS_MSG(
...
int i2c_master_wait_for_inactivity(const kb900x_config_t *config)
{
    ...
    for (; retry < nb_retry; retry++) {
        int ret = kb900x_read_register(config, ee_IC_STATUS, &result);
                            ...
                            const uint8_t *data)
{
    ...
    int ret = kb900x_read_register(config, ee_IC_RAW_INTR_STAT, &result);
    ...
    if ((result & mask) != 0) {
        ...
        uint32_t tmp;
        kb900x_read_register(config, tx_abrt_clr, &tmp);
...
int unlock_eeprom(const kb900x_config_t *config)
{
    ...
    int ret = kb900x_read_register(config, cfg_top_vd_bump_0, &prev_val_1);
    CHECK_SUCCESS_MSG(ret, "Something went wrong while interracting with the I2C master interface");
    ret = kb900x_read_register(config, cfg_top_vd_bump_1, &prev_val_2);
    CHECK_SUCCESS_MSG(ret, "Something went wrong while interracting with the I2C master interface");
    // Configure
    ret = write_field(config, cfg_top_vd_bump_0, field_width, 0, 1 << shift);
    CHECK_SUCCESS_MSG(ret, "Something went wrong while interracting with the I2C master interface");
    ret = write_field(config, cfg_top_vd_bump_0, field_width, snd_field_position, 1 << shift);
    CHECK_SUCCESS_MSG(ret, "Something went wrong while interracting with the I2C master interface");
    // Send ticks
    for (uint8_t i = 0; i < nb_ticks_to_send; i++) {
        ret = write_field(config, cfg_top_vd_bump_1, field_width, 0, 1 << shift);
        CHECK_SUCCESS_MSG(ret, "Something went wrong while sending ticks HIGH");
        ret = write_field(config, cfg_top_vd_bump_1, field_width, 0, 0 << shift);
        CHECK_SUCCESS_MSG(ret, "Something went wrong while sending ticks LOW");
    }
    // Deconfigure
    ret = kb900x_write_register(config, cfg_top_vd_bump_0, prev_val_1);
    CHECK_SUCCESS_MSG(ret, "Something went wrong while interracting with the I2C master interface");
    ret = kb900x_write_register(config, cfg_top_vd_bump_1, prev_val_2);
    CHECK_SUCCESS_MSG(ret, "Something went wrong while interracting with the I2C master interface");
    return KB900X_E_OK;
}
